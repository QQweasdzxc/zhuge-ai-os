...
test("Shared Task Drawer has no domain, Cloud, authorization, or WorkLog ownership", () => {
  ...
  assert.match(read("docs/20_MODULES/SHARED_TASK_DRAWER_COMPATIBILITY_ASSESSMENT.md"), /Formal Template C AI Board and\s+WorkTodo Checklist operations use the Shared Action Contract and the Board\s+canonical data path\./);
...
test("WorkLog compatibility assessment preserves Date and Calendar as WorkLog domain fields", () => {
  const assessment = read("docs/20_MODULES/SHARED_TASK_DRAWER_COMPATIBILITY_ASSESSMENT.md");
