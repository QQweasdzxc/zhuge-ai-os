...
test("Engineering Data Health is read-only and reports duplicate and stale findings", async () => {
  const gateway = {
    select: async table => {
      ...
      if (table === "engineering_knowledge") return [{ knowledge_code: "TASK-026-SYSTEM-MAP", title: "系統藍圖", summary: "old", knowledge_type: "system_map", updated_at: "2026-08-01T00:00:00Z" }];
      if (table === "engineering_checklist_items") return [];
      return [];
    }
  };
  const report = await Board.runHealthCheck({ gateway });
