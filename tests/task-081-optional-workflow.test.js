...
test("TASK-081 source exposes explicit detach and preserves optional movement", async () => {
  assert.match(service, /board_c_detach_workflow_and_move_task_v1/);
  assert.match(service, /input\.detachWorkflow === true/);
  assert.match(service, /input\.adoptWorkflow === true/);
  assert.match(service, /p_workflow_mode: input\.workflowMode \|\| "published"/);
  assert.match(adapters, /detachWorkflow: payload\.detachWorkflow === true/);
  ...
  const workflow = BoardReadService.createWorkflowCapability({
    gateway: {
      ...
      async rpc(name, args) {
        ...
        if (name === "board_c_detach_workflow_and_move_task_v1") return { action: name, workflow_bound: false };
  ...
  const result = await workflow.moveWorkspaceDecision({
    ...
    detachWorkflow: true,
    decisionNote: "Leave published workflow explicitly",
    idempotencyKey: "task-081-detach"
  });
  assert.equal(result.raw.workflow_bound, false);
  assert.deepEqual(calls, [{
    name: "board_c_detach_workflow_and_move_task_v1",
    args: {
      p_task_id: "task-081",
      p_target_workspace_id: "paused-workspace",
      p_reason: "Leave published workflow explicitly",
...
test("TASK-081 unbound movement does not auto-adopt a published workflow", async () => {
  ...
  const workflow = BoardReadService.createWorkflowCapability({
    gateway: {
      ...
      async rpc(name, args) {
        calls.push({ name, args });
        if (name === "board_c_workflow_get") return { state: { published_workflow_version_id: "workflow-1" }, published: { id: "workflow-1" } };
        if (name === "board_c_workflow_resolve_task") return { state: "workflow_not_configured" };
        if (name === "board_c_reconcile_workspace_decision_v2") return { workflow_bound: false, workflow_optional: true };
        throw new Error(`Unexpected RPC: ${name}`);
      }
    },
    boardInstanceId: "consumer-board",
