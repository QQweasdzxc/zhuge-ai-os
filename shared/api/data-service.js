    ...
    try {
      const safeLoad = async (label, loader, fallback) => {
        try { return await loader(); }
          ...
          const optionalDomains = new Set(["knowledge", "knowledge_units", "knowledge_candidates", "work_models", "work_journal", "conversation"]);
          if (optionalDomains.has(label)) optionalErrors.push(message);
      ...
      // Core WorkLog domains must be applied before optional AI/Knowledge processing.
      // A failure in Conversation or Work Memory must never hide cloud tasks or month entries.
      ...
      // Conversation is an optional AI context domain. It has its own health state
      // and must never change the WorkLog Cloud status or block core data rendering.
      await this.loadConversation();
      purgeCloudBackedLocalData();
      if (optionalErrors.length) console.warn("Optional cloud domains unavailable", optionalErrors);
