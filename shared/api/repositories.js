...
const SupabaseRepository = {
  ...
  async request(path, options = {}) {
    ...
    if (!res.ok) {
      ...
      if (this.shouldRefreshAfter(firstError)) {
        ...
        if (!refreshed?.access_token || accessTokenNeedsRefresh(0)) {
          const authError = new Error("登入工作階段已逾時，請重新整理頁面後重新登入。");
          authError.code = "AUTH_SESSION_EXPIRED";
          authError.cause = firstError;
          throw authError;
        }
        res = await run();
  ...
  async saveTasks(items = []) {
    ...
    // Cloud-first safety rule: absence from the current UI list is not a delete command.
    // Tasks may only be archived/deleted through an explicit user action. This prevents
    // a partial hydration or temporary empty screen from soft-deleting cloud history.
