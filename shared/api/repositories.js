...
const SupabaseRepository = {
  ...
  async loadEntries(month = monthKey()) {
    const userUuid = currentUserUuid();
    if (!userUuid) throw new Error("缺少使用者 UUID，無法載入工時");
    const rows = await this.select(
      "work_entries",
      `?select=*&user_uuid=eq.${encodeURIComponent(userUuid)}&work_date=gte.${month}-01&work_date=lt.${nextMonthKey(month)}-01&status=neq.deleted&deleted_at=is.null&order=started_at.asc`
    );
    return rows || [];
  },
