...
<div id="consumerCreateModal" class="modalback" aria-hidden="true">
 <div class="modal board-create-drawer" role="dialog" aria-modal="true" aria-labelledby="consumerCreateTitle">
  ...
  <div class="modalbody">
   <p class="board-create-description">輸入看板名稱、看板代號與正式歸屬，系統會在同一個受控交易中建立獨立資料範圍、四個預設工作區，並自動採用目前已發布的 C 母版。</p>
   ...
   <div class="field"><label for="consumerBoardScope">正式歸屬</label><input id="consumerBoardScope" type="text" maxlength="64" placeholder="例如：hr 或 procurement" autocomplete="off"><div class="hint">建立後會以此正式路由識別 Consumer；同一個歸屬同時間只能有一個啟用中的 Consumer。</div></div>
