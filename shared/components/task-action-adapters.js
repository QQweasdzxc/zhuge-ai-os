...
(function (root, factory) {
  ...
  function createCTemplateAdapter({ task, service } = {}) {
    ...
    return {
      consumer: "c_mdtk",
      sharedDrawerContract: Object.freeze({ ownsDrawer: false, viewModel: "toSharedViewModel" }),
      capabilities: Object.freeze({
        progressNoteAttachment: true,
        agreementSchedule: true,
        taskChecklist: true,
      ...
      actions: {
        ...
        deleteChecklist: payload => required(service, "deleteTaskChecklistItem")(payload.id),
        updateGovernanceChecklist: payload => required(service, "updateChecklistItem")({ id: payload.id, state: payload.state, evidenceNote: payload.evidenceNote, evidenceRef: payload.evidenceRef }),
        setAgreementSchedule: payload => required(service, "setAgreementSchedule")({
          taskId: payload.taskId || taskId,
          mode: payload.mode,
          startDate: payload.startDate,
          endDate: payload.endDate
  ...
  function create(options = {}) {
    return options.cTemplate
      ? createCTemplateAdapter(options)
      : options.workTodo
      ? createWorkTodoAdapter(options)
      : createAiBoardAdapter(options);
  }

  return Object.freeze({ create, createAiBoardAdapter, createWorkTodoAdapter, createCTemplateAdapter, rawWorkTodoAttachment });
});
