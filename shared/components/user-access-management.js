...
(function (root, factory) {
  ...
  function renderShell() {
    return `<section class="user-access-management" data-user-access-management aria-labelledby="user-access-management-title">
      <div class="user-access-management-heading"><div><p class="eyebrow">Creator / Owner</p><h2 id="user-access-management-title">👥 User Access Management</h2><p class="muted">Google 登入身份與 Zhuge AI OS 使用申請。申請歷史採 append-only 保留。</p></div><button class="btn2" type="button" data-access-refresh>重新整理</button></div>
      <div class="user-access-management-status" data-access-management-status role="status" aria-live="polite">正在讀取申請紀錄…</div>
  ...
  function rowMarkup(item, note = "") {
    ...
    return `<article class="user-access-application" data-application-id="${id}">
      ...
      <div class="user-access-note-row"><label>Creator 私人備註<input class="input" type="text" maxlength="1000" value="${escape(note)}" data-creator-note></label><button class="btn2" type="button" data-save-creator-note="${id}">儲存備註</button></div>
      <div class="user-access-application-actions"><button class="btn2" type="button" data-view-application-history="${id}">查看歷史</button></div>
  ...
  async function mount(rootNode, options = {}) {
    ...
    const load = async () => {
      ...
      try {
        ...
        setStatus(`${state.applications.length} 筆申請紀錄；${pending} 筆待審核。`);
      } catch (error) {
        listNode.innerHTML = "";
