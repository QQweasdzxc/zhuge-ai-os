...
(function (root) {
  ...
  function renderMfaUnlock(context, access) {
    ...
    const paint = () => {
      ...
      let body = `<div class="board-access-message">AI Board 需要額外的 Google Authenticator 驗證。驗證只會解鎖目前帳號的 AI Board，不會變更登入身份。</div>${error}`;
      if (state.mode === "loading") {
        body += `<div class="board-access-progress" role="status">正在檢查安全驗證…</div>`;
      } else if (state.mode === "enrollment_required") {
        body += `<div class="board-access-actions"><button class="btn primary" type="button" id="boardMfaEnroll" ${state.busy ? "disabled" : ""}>設定 Google Authenticator</button></div>`;
      } else {
        const qr = state.qrCode ? `<div class="board-access-qr"><img src="${esc(state.qrCode)}" alt="Google Authenticator 設定 QR Code"></div>` : "";
        const secret = state.secret ? `<details class="board-access-secret"><summary>無法掃描？查看設定碼</summary><code>${esc(state.secret)}</code></details>` : "";
        body += `${qr}${secret}<form class="board-access-form" id="boardMfaForm">
          <label for="boardMfaCode">Google Authenticator 驗證碼</label>
          <input id="boardMfaCode" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" placeholder="輸入 6 位數驗證碼" required>
          <button class="btn primary" type="submit" ${state.busy ? "disabled" : ""}>${state.mode === "enroll" ? "完成設定並開啟 AI Board" : "驗證並開啟 AI Board"}</button>
        </form>`;
      }
      renderAccessState({ title: "AI Board 安全驗證", message: "這是受保護的工程治理區域。", kind: "mfa", body });
