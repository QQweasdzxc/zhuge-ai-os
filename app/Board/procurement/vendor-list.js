(function(){
  ...
  const EDITABLE_FIELDS=[
    {key:"orderDate",label:"日期"},{key:"company",label:"公司"},{key:"purchaseNo",label:"採購單號"},{key:"vendorName",label:"廠商名稱"},{key:"taxId",label:"統編"},{key:"contactMailLegacy",label:"原始通報 Email"},{key:"paymentTerms",label:"付款方式"},{key:"products",label:"產品／類型",wide:true,textarea:true},{key:"integritySignedAt",label:"誠信簽署日期"},{key:"integrityOriginal",label:"誠信原始資料",wide:true,textarea:true},{key:"csrSelfAssessment",label:"CSR 自評",wide:true,textarea:true},{key:"csrOriginal",label:"CSR 原始資料",wide:true,textarea:true},{key:"phone",label:"電話"},{key:"contactName",label:"聯絡人"},{key:"mobile",label:"手機"},{key:"email",label:"Email",type:"email"},{key:"project",label:"專案",wide:true},{key:"contracted",label:"是否簽約"},{key:"insured",label:"是否投保"}
  ];
  const EDITABLE_KEYS=EDITABLE_FIELDS.map(field=>field.key);
  const esc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
  const editorFields=EDITABLE_FIELDS.map(field=>`<label${field.wide?' class="wide"':''}>${field.label}${field.textarea?`<textarea name="${field.key}" rows="3"></textarea>`:`<input name="${field.key}"${field.type?` type="${field.type}"`:''}>`}</label>`).join("");
  root.innerHTML=`<section class="vendor-page">
    <div class="vendor-toolbar"><div><h2>廠商清單</h2><p>與 Google Sheet 共用同一份資料；此頁不建立第二套廠商主檔。</p></div><div class="vendor-sync" data-vendor-sync>尚未同步</div></div><div class="vendor-progress" data-vendor-progress hidden><div class="vendor-progress-head"><strong data-vendor-progress-label>準備同步…</strong><span data-vendor-progress-percent>0%</span></div><div class="vendor-progress-track"><div class="vendor-progress-bar" data-vendor-progress-bar></div></div></div>
  ...
  const $=s=>root.querySelector(s); const body=$('[data-vendor-body]'),search=$('[data-vendor-search]'),company=$('[data-vendor-company]'),product=$('[data-vendor-product]'),sync=$('[data-vendor-sync]'),dialog=$('[data-vendor-dialog]'),form=dialog.querySelector('form'),progress=$('[data-vendor-progress]'),progressLabel=$('[data-vendor-progress-label]'),progressPercent=$('[data-vendor-progress-percent]'),progressBar=$('[data-vendor-progress-bar]');
  ...
  async function load(){setSync("同步中…","loading");setProgress({phase:"start",percent:1,message:"準備連線 Google Sheet…"});try{rows=await service.list({onProgress:setProgress});fillCompanies();render();setProgress({phase:"done",percent:100,message:`同步完成 · ${rows.length} 筆廠商`});setSync(`已同步 · ${new Date().toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit"})}`,"ok");finishProgress();}catch(e){console.error(e);setProgress({phase:"error",percent:0,message:e.message||"同步失敗"});setSync(e.message||"同步失敗","error");body.innerHTML=`<tr><td colspan="8" class="vendor-error">${esc(e.message||"無法讀取 Google Sheet")}</td></tr>`;}}
  function openEditor(rowNumber){selected=rows.find(r=>r.rowNumber===Number(rowNumber));if(!selected)return;$('[data-vendor-id]').textContent=selected.vendorId||`Sheet 第 ${selected.rowNumber} 列`;$('[data-vendor-title]').textContent=selected.vendorName||"廠商資料";EDITABLE_KEYS.forEach(k=>{const field=form.elements.namedItem(k);if(field)field.value=selected[k]||"";});$('[data-vendor-save-state]').textContent="";dialog.showModal();}
  async function save(){if(!selected)return;const btn=$('[data-vendor-save]'),state=$('[data-vendor-save-state]');btn.disabled=true;state.textContent="同步中…";const patch={};EDITABLE_KEYS.forEach(k=>{const field=form.elements.namedItem(k);patch[k]=field?.value.trim()||"";});try{const updated=await service.update(selected.vendorId,patch,{onProgress:info=>{setProgress(info);state.textContent=info.message||"同步中…";}});const index=rows.findIndex(r=>r.vendorId===updated.vendorId);if(index>=0)rows[index]=updated;else rows.push(updated);selected=updated;fillCompanies();render();setSync(`已同步 · ${new Date().toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit"})}`,"ok");state.textContent="已寫回 Google Sheet";finishProgress();setTimeout(()=>dialog.close(),450);}catch(e){console.error(e);setProgress({phase:"error",percent:0,message:e.message||"同步失敗"});state.textContent=e.message||"同步失敗";}finally{btn.disabled=false;}}
