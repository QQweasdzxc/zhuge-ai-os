(function(){
  ...
  const $=s=>root.querySelector(s); const body=$('[data-vendor-body]'),search=$('[data-vendor-search]'),company=$('[data-vendor-company]'),product=$('[data-vendor-product]'),sync=$('[data-vendor-sync]'),dialog=$('[data-vendor-dialog]'),progress=$('[data-vendor-progress]'),progressLabel=$('[data-vendor-progress-label]'),progressPercent=$('[data-vendor-progress-percent]'),progressBar=$('[data-vendor-progress-bar]');
  ...
  async function save(){if(!selected)return;const btn=$('[data-vendor-save]'),state=$('[data-vendor-save-state]');btn.disabled=true;state.textContent="同步中…";const patch={};["vendorName","company","taxId","contactName","phone","mobile","email","paymentTerms","products","project"].forEach(k=>patch[k]=dialog.elements[k].value.trim());try{const updated=await service.update(selected.rowNumber,patch,{onProgress:info=>{setProgress(info);state.textContent=info.message||"同步中…";}});const index=rows.findIndex(r=>r.rowNumber===updated.rowNumber);if(index>=0)rows[index]=updated;else rows.push(updated);fillCompanies();render();setSync(`已同步 · ${new Date().toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit"})}`,"ok");state.textContent="已寫回 Google Sheet";finishProgress();setTimeout(()=>dialog.close(),450);}catch(e){console.error(e);setProgress({phase:"error",percent:0,message:e.message||"同步失敗"});state.textContent=e.message||"同步失敗";}finally{btn.disabled=false;}}
