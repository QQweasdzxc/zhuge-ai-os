-- Investment IVTK projection activation compatibility
-- Scope: keep the one-time identity repair no-op once the canonical initial
-- identities are present, even after later idempotent projections add cards.
-- No Investment data, card identity, or work_code is changed by this guard.
...
begin
  ...
  if v_canonical_count = 8
     and v_legacy_projection_count = 8
     and v_projection_count >= 8 then
