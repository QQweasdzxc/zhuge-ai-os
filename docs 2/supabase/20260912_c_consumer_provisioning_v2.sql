-- Module C canonical Consumer provisioning v2.
--
-- This is an additive, idempotent provisioning contract.  It creates one
-- Board Instance and its own data/workflow scope in one transaction, while
-- reusing the existing C workflow draft/publish contracts.  It never copies
-- the C source, creates cards, or changes an existing Consumer.

begin;

create table if not exists private.board_c_consumer_provision_idempotency (
...
alter table private.board_c_consumer_provision_idempotency enable row level security;
revoke all on table private.board_c_consumer_provision_idempotency from public, anon, authenticated;
...
begin
  ...
  -- Serialise retries for the same logical provisioning request before the
  -- idempotency row is read or any identity is created.
  ...
  -- The existing low-level creator supplies the Board Identity.  The outer
  -- function performs the governance check and keeps all following work in
  -- the same transaction so a partial Consumer cannot survive an error.
  ...
  if p_workflow_blueprint is null then
    -- This is a per-Instance starter definition, not a C Mother workflow.
    -- Each new Consumer owns the resulting immutable version and can edit a
    -- later draft through the existing Workflow Settings contract.
...
comment on table private.board_c_consumer_provision_idempotency is
  'Private idempotency records for the single-transaction Module C Consumer provisioning contract.';
comment on function public.board_provision_c_consumer_v2(text, text, text, text, jsonb, jsonb, text) is
  'Canonical Module C Consumer provisioning: Board Instance, independent workspaces, C release adoption, and optional Instance-owned Published Workflow in one transaction.';
