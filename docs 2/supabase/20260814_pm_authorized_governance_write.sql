-- PM-authorized Governance Write Path
--
-- Reuses the existing Engineering Actor identity, board_create_task(),
-- write_engineering_checkpoint(), and engineering_activity_log.  The table
-- below stores only a one-time PM authorization capability hash; it is not a
-- second identity or a second Governance Source of Truth.

begin;

-- Extend the existing audit vocabulary by one canonical entity type for the
-- authorization ledger. Existing activity categories remain unchanged.
...
create table if not exists public.engineering_governance_authorizations (
  authorization_id uuid primary key default extensions.gen_random_uuid(),
  token_hash text not null unique,
  operation text not null check (operation in ('create_task_contract', 'update_task_contract', 'update_checkpoint', 'register_artifact')),
  request_hash text not null,
  authorized_actor text not null default 'GPT' check (authorized_actor = 'GPT'),
  ...
  pm_note text,
  constraint engineering_governance_authorization_expiry_check
    check (expires_at > issued_at and expires_at <= issued_at + interval '15 minutes')
);

create index if not exists engineering_governance_authorizations_active_idx
  on public.engineering_governance_authorizations (operation, authorized_actor, expires_at)
  where used_at is null and revoked_at is null;

alter table public.engineering_governance_authorizations enable row level security;
revoke all on public.engineering_governance_authorizations from public, anon, authenticated, service_role;

-- Keep the operation allowlist current when this idempotent source is rerun
-- against the already-created authorization ledger.
...
alter table public.engineering_governance_authorizations
  ...
  check (operation in ('create_task_contract', 'update_task_contract', 'update_checkpoint', 'register_artifact'));

-- PM / QJC issues a bounded, payload-bound, one-time authorization.  GPT can
-- receive the returned opaque token, but cannot issue one because this path
-- requires the authenticated Engineering owner identity.
...
begin
  ...
  if operation_value not in ('create_task_contract', 'update_task_contract', 'update_checkpoint', 'register_artifact') then
...
-- Only the authenticated QJC / PM path can issue a capability.
...
-- The Edge Function's server-only service path consumes the capability and
-- dispatches only to existing controlled RPCs.  No direct table DML is
-- granted to GPT, Co, authenticated users, anon, or the browser.
...
-- This executor is intentionally invisible to normal roles and browser RPC
-- clients.  The Edge Function service runtime is the only caller.
