-- AI Board Batch #2 follow-up: formal TASK usage scenario.
-- Approved minimum schema change. Existing rows remain NULL and are shown as
-- 「尚未補充使用情境」; no scenario is invented for historical TASKs.
begin;

alter table public.board_tasks
  add column if not exists usage_scenario text;

-- Replace the previous five-argument RPC so new TASK creation persists the
-- Development Contract usage scenario through the Shared Gateway.
drop function if exists public.board_create_task(text, text, text, text, text);
...
begin
  ...
  insert into public.board_tasks (
    ...
    'ready', 'Co', actor_id_value, now(), now()
