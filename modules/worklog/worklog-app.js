...
function workMemoryListRowMarkup(item = {}) {
  ...
  const actionMarkup = workMemoryMergeMode
    ? `<button class="btn2 ${selected ? "selected" : ""}" type="button" data-toggle-work-memory-merge="${escapeHtml(item.name)}">${selected ? "✓ 已選取" : "＋ 選取合併"}</button>`
  ...
  return `<article class="work-memory-list-row work-memory-confirmed-card ${selected ? "is-merge-selected" : ""}">
    ...
    <div class="work-memory-list-meta"><span>預設工時 ${escapeHtml(formatHumanDuration(Number(item.defaultDurationMinutes || 0) / 60))}</span><span>熟悉度 ${escapeHtml(workMemoryFamiliarityBars(item.familiarityScore))}</span><span>${item.usageCount ? `使用 ${item.usageCount} 次` : "尚未使用"}</span></div>
