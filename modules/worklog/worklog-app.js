...
function normalizeWorkProfile(value = {}, baseProfile = profile) {
  // Cloud rows are optional. A missing user_work_profiles row is returned as null,
  // so normalize it to a safe object before reading any properties.
  ...
  const result = {
    ...
    profileCompleted: Boolean(source.profileCompleted ?? source.profile_completed ?? seed.profileCompleted),
...
/*
 * WorkTodo Shared Task Drawer integration.
 *
 * The adapter only renders a presentation contract.  These bindings keep
 * every mutation on the existing WorkTodo DataService / Repository path.
 */
function workTodoSharedTask(taskId = taskJournalTaskId) {
  return tasks.find(item => String(item.id) === String(taskId)) || null;
}

async function saveWorkTodoTaskPatch(taskId, patch = {}, message = "待辦事項已更新至 Cloud") {
  const current = workTodoSharedTask(taskId);
  if (!current) return;
  const previous = tasks;
  const next = normalizeTask({ ...current, ...patch, updatedAt: new Date().toISOString() });
  tasks = tasks.map(item => item.id === current.id ? next : item);
  try {
    await DataService.saveTasksNow(tasks);
    toast(message);
    render("worktodo-shared-task-updated");
  } catch (error) {
    tasks = previous;
    console.error("WorkTodo Shared Task update failed", { error, taskId, patch });
    toast(error.message || "待辦事項更新失敗，未完成儲存");
    render("worktodo-shared-task-update-failed");
  }
}
...
function bindWorkTodoInlineField(root, task) {
  root.querySelectorAll("[data-worktodo-edit-field]").forEach(button => {
    button.onclick = () => {
      const field = button.dataset.worktodoEditField;
...
function bindWorkTodoTitleEditor(root, task) {
  root.querySelector("[data-task-title-edit]")?.addEventListener("click", () => {
    const row = root.querySelector("[data-shared-task-title-heading]")?.closest(".shared-task-drawer-title-row");
    const heading = row?.querySelector("[data-shared-task-title-heading]");
    if (!row || !heading) return;
    row.innerHTML = `<div class="worktodo-shared-title-editor"><span class="shared-task-drawer-title-code">${escapeHtml(task.id)}</span><span class="shared-task-drawer-title-separator" aria-hidden="true">｜</span><input class="shared-task-drawer-title-input" data-worktodo-title-editor value="${escapeHtml(task.title)}" aria-label="TASK 主旨"><span class="shared-task-drawer-title-actions"><button class="btn2" type="button" data-worktodo-title-cancel>取消</button><button class="btn" type="button" data-worktodo-title-save>儲存</button></span></div>`;
    row.querySelector("[data-worktodo-title-editor]")?.focus();
    row.querySelector("[data-worktodo-title-cancel]")?.addEventListener("click", () => render("worktodo-title-edit-cancel"));
    row.querySelector("[data-worktodo-title-save]")?.addEventListener("click", () => {
      const nextTitle = row.querySelector("[data-worktodo-title-editor]")?.value?.trim() || "";
      if (!nextTitle) return toast("TASK 主旨不可為空白");
      saveWorkTodoTaskPatch(task.id, { title: nextTitle }, "TASK 主旨已更新至 Cloud");
    });
  });
}

function bindWorkTodoControls(root, task) {
  const progress = root.querySelector("[data-worktodo-progress]");
  const output = root.querySelector("[data-worktodo-progress-output]");
  progress?.addEventListener("input", () => { if (output) output.value = `${taskProgressValue(progress.value)}%`; });
  root.querySelector("[data-worktodo-save-controls]")?.addEventListener("click", () => {
    const status = normalizeTaskStatus(root.querySelector("[data-worktodo-status]")?.value || task.status, task);
    const nextProgress = status === "completed" ? 100 : taskProgressValue(root.querySelector("[data-worktodo-progress]")?.value || task.progress);
    saveWorkTodoTaskPatch(task.id, {
      status,
      progress: nextProgress,
      priority: root.querySelector("[data-worktodo-priority]")?.value || task.priority,
      dueDate: root.querySelector("[data-worktodo-due-date]")?.value || "",
      userPinned: root.querySelector("[data-worktodo-pin]")?.checked === true,
      completedAt: status === "completed" && task.status !== "completed" ? new Date().toISOString() : (status === "completed" ? task.completedAt : ""),
      completedBy: status === "completed" ? (task.completedBy || currentUserUuid()) : ""
    }, "工作屬性已更新至 Cloud");
  });
}

function bindWorkTodoSharedDrawer() {
  const root = document.querySelector("[data-worktodo-shared-drawer]");
  if (!root) return;
  const task = workTodoSharedTask(root.dataset.worktodoTaskId);
  if (!task) return;
  root.querySelectorAll("[data-shared-task-drawer-close]").forEach(button => {
    button.onclick = event => {
      ...
      taskJournalEditingEntryId = null;
      taskJournalLoading = false;
      render("worktodo-shared-drawer-close");
    };
  });
  ...
  root.querySelectorAll("[data-worktodo-journal-edit]").forEach(button => {
    button.onclick = () => {
      ...
      if (!entry) return toast("找不到這筆工作進度");
  ...
  root.querySelector("[data-journal-cancel]")?.addEventListener("click", () => {
    taskJournalDraft = null;
    taskJournalEditingEntryId = null;
    render("worktodo-journal-edit-cancel");
  });
}
...
function refreshCloudSyncStatusDisplay() {
  ...
  const statusLabel = sidebarSyncStatusLabel();
  const syncTime = cloudSync.lastSyncedAt ? fmt(cloudSync.lastSyncedAt) : "尚未同步";
  const statusNode = box.querySelector("strong");
  const labelNode = box.querySelector("span");
  const timeNode = box.querySelector("time");

  // The sync status is refreshed by several background services. Replacing the
  // entire innerHTML on every refresh caused a child-list mutation and a full
  // sidebar repaint even when the displayed values had not changed. On macOS
  // Chrome/Safari this appeared as high-frequency screen flicker. Preserve the
  // existing nodes and update only values that actually changed.
  let changed = false;
  if (statusNode && statusNode.textContent !== statusLabel) {
    statusNode.textContent = statusLabel;
    changed = true;
  }
  if (labelNode && labelNode.textContent !== "最後同步") {
    labelNode.textContent = "最後同步";
    changed = true;
  }
  if (timeNode && timeNode.textContent !== syncTime) {
    timeNode.textContent = syncTime;
    changed = true;
  }

  // Compatibility fallback for malformed/legacy markup. This path runs once,
  // not on every status poll.
  if (!statusNode || !labelNode || !timeNode) {
    box.replaceChildren();
    const strong = document.createElement("strong");
    const span = document.createElement("span");
    const time = document.createElement("time");
    strong.textContent = statusLabel;
    span.textContent = "最後同步";
    time.textContent = syncTime;
    box.append(strong, span, time);
    return true;
  }
...
function normalizeWorkspaceState() {
  // The former hidden system-templates route is now a compatibility alias
  // for the single Control Center management surface.  Do not restore a
  // second Template Management UI from persisted browser state.
...
function developerConsoleMarkup() {
  ...
  return `<section class="developer-console" aria-label="System Information"><details class="desktop-secondary-details"><summary><span>系統技術細節</span><small>Build／Version／Schema／同步時間</small></summary><div class="developer-console-content control-grid system-info-grid">${cards.map(([id, title, status, detail]) => `<article class="service-card system-info-card" data-system-info-card="${id}"><div class="system-info-card-main"><div class="system-info-card-head"><div><h3>${escapeHtml(title)}</h3><b>${escapeHtml(status)}</b></div></div>${detail ? `<div class="muted system-info-card-detail">${escapeHtml(detail)}</div>` : ""}</div></article>`).join("")}</div></details></section>`;
}
...
function management() {
  return `<section class="panel control-center management-center" aria-label="管理功能內容"><section class="management-first-screen" aria-labelledby="management-first-screen-title"><div><p class="eyebrow">管理功能</p><h2 id="management-first-screen-title">先處理需要你決定的事情</h2><p class="muted">從工作看板、使用者存取與系統藍圖開始；技術細節只在需要時展開。</p></div><div class="management-summary-grid"><div class="management-summary-card"><strong>工作與治理</strong><small>查看正式工作、準則與交接狀態</small></div><div class="management-summary-card"><strong>使用者存取</strong><small>查看申請、權限與待處理項目</small></div><div class="management-summary-card"><strong>系統理解</strong><small>需要時再查看架構與技術資料</small></div></div></section><section class="control-center-management-column" aria-labelledby="management-entry-title"><div class="control-center-entries"><div class="control-center-entry-heading"><div><h2 id="management-entry-title">管理入口</h2><p class="muted">進入工作看板、工程準則與系統藍圖。</p></div></div><div class="control-center-entry-grid">${controlCenterEntryMarkup()}${templateManagementMarkup()}</div><div data-user-access-management></div></div></section></section>`;
}
...
function sync() {
  ...
  const services = [
    ["Google 帳號", session ? "🟢 已登入" : "⚪ 尚未登入", "", session ? "可以使用需要登入的工作空間" : "尚未登入，部分功能無法使用"],
    ["Google Drive", googleState, driveAction, driveAuthorized ? "可讀取你明確選取的文件" : "尚未授權，不會讀取文件"],
    ["Cloud Sync", cloudSyncLabel(), "", "影響跨裝置資料是否保持同步"],
    ["AI 引擎", "🟢 正常", "", "可使用既有 AI 助手與知識整理"],
    ["Supabase", session ? "🟢 已連線" : "⚪ 尚未登入", "", "影響正式 Cloud 資料讀取與儲存"],
    ["本機資料", "🟢 正常", "", "保留尚未同步的本機工作內容"]
  ];
  const online = session ? "目前可以使用" : "需要登入後才能使用";
  return `<section class="panel control-center" aria-label="控制台內容"><section class="control-center-first-screen" aria-labelledby="control-center-first-screen-title"><div><p class="eyebrow">控制台</p><h2 id="control-center-first-screen-title">${online}</h2><p class="muted">先看是否有會影響工作的問題；Build、Schema 與同步細節放在下方展開。</p></div><div class="control-center-summary-grid"><div class="control-center-summary-card"><strong>${session ? "🟢 工作空間可用" : "⚪ 尚未登入"}</strong><small>${session ? "登入狀態正常" : "請先登入 Zhuge AI OS"}</small></div><div class="control-center-summary-card"><strong>${cloudSyncLabel()}</strong><small>跨裝置資料同步狀態</small></div><div class="control-center-summary-card"><strong>🟢 AI 可使用</strong><small>既有 AI 助手與知識能力</small></div></div></section><section class="control-center-status-column" aria-labelledby="control-center-status-title"><h2 id="control-center-status-title" class="dashboard-section-label">系統狀態</h2><h3 class="desktop-status-subheading">服務狀態</h3><div class="control-grid">${services.map(([name, state, action, impact]) => `<div class="service-card"><div><h3>${escapeHtml(name)}</h3><b>${escapeHtml(state)}</b><small class="muted">${escapeHtml(impact)}</small></div>${action}</div>`).join("")}</div>${developerConsoleMarkup()}</section></section>`;
}
...
function libraryView() {
  ...
  const body = knowledgeFoundationNotInitialized
    ...
    : (visibleItems.length ? visibleItems.map(item => {
      ...
      return `<article class="entry knowledge-card knowledge-card-tile knowledge-card-status-${escapeHtml(item.processingStatus || "uploaded")}"><div class="entry-main"><div class="knowledge-card-title"><b>🪶 ${escapeHtml(item.title)}</b><span class="knowledge-card-category">${escapeHtml(item.category)}</span></div><div class="knowledge-card-status-line"><span class="knowledge-status-dot"></span><span>${escapeHtml(knowledgeLearnedLabel(item.processingStatus))}</span><span>｜${escapeHtml(providerLabel)}｜${escapeHtml(knowledgeSourceTypeLabel(item.sourceType))}</span></div><small class="knowledge-card-description">${escapeHtml(item.description || "這份文件會幫我更懂你的工作。")}</small><details class="knowledge-card-details"><summary><span>查看理解與狀態</span><small>流程／規則／可協助工作</small></summary><div class="knowledge-card-details-content"><div class="source-path"><b>我目前理解出的工作：</b></div><ul class="knowledge-result-list">${capabilityList}</ul><div class="source-path">理解 ${counts.summary}｜流程 ${counts.process}｜規則 ${counts.rule}｜可協助工作 ${counts.recommendation}</div>${item.processingStatus === "failed" && item.intelligenceError ? `<div class="source-path knowledge-error-copy">我讀不懂的原因：${escapeHtml(item.intelligenceError)}</div>` : ""}</div></details></div><div class="knowledge-card-toolbar"><button class="btn2" ${viewDisabled} data-view-knowledge-result="${item.id}">查看我的理解</button>${item.processingStatus === "processed" ? `<button class="btn green" data-verify-library="${item.id}">✓ 接受我的理解</button>` : ""}<details class="knowledge-card-more"><summary aria-label="更多操作">⋯ 更多</summary><div class="knowledge-card-more-menu"><button class="btn2" data-refresh-library="${item.id}">${escapeHtml(sourceAction)}</button>${previewButton}<button class="btn2" data-edit-library="${item.id}">✏️ 調整我的理解</button><button class="btn2" data-archive-library="${item.id}">封存</button><button class="btn2 danger" data-del-library="${item.id}">刪除</button></div></details></div></article>`;
  ...
  return `<section class="panel" style="margin-top:18px" data-knowledge-library><div class="panel-head"><div><p class="eyebrow">Knowledge</p><h2>📚 我的工作知識</h2><div class="muted">先看文件名稱、來源與整理狀態；需要時再展開理解內容。</div></div>${addButton}</div>${legacyBlock}${filters}<div class="knowledge-library-count muted">顯示 ${visibleItems.length} / ${library.length} 份知識</div><div class="library-list knowledge-card-grid">${body}</div></section>`;
}
...
function settings() {
  ...
  const settingsFirstScreen = `<div class="settings-first-screen"><div><p class="eyebrow">設定</p><h2>管理帳號、偏好與工作設定</h2><div class="muted">先從帳號與安全開始；低頻的工作記憶與系統資訊需要時再展開。</div></div><nav class="settings-section-index" aria-label="設定分類"><a href="#settings-account-security">帳號與安全</a><a href="#settings-preferences">個人偏好</a><a href="#settings-work-settings">工作設定</a><a href="#settings-advanced-system">進階系統</a></nav></div>`;
  const workSection = `<details class="settings-secondary-section settings-work-section" id="settings-work-settings"><summary><span>我的工作與 AI 建議</span><small>低頻設定與工作記憶</small></summary><div class="settings-secondary-section-content">${workMemoryPage({ compact: true })}</div></details>`;
  const advancedSection = `<details class="settings-secondary-section settings-advanced-section" id="settings-advanced-system"><summary><span>進階系統</span><small>重設、登出與版本資訊</small></summary><div class="settings-secondary-section-content"><div class="settings-actions"><button class="btn2 settings-reset-action" id="resetProfile">重新初次認識</button><button class="btn settings-logout-action" id="logoutBtn">登出</button></div><div class="settings-version"><span>About／版本</span><strong>${VERSION}</strong></div></div></details>`;
  return `<section class="panel settings-page" style="margin-top:18px"><div class="settings-page-heading"><div><h2>⚙️ 設定</h2><div class="muted">管理個人工作身分與 Zhuge AI OS 偏好</div></div></div>${settingsFirstScreen}<div class="settings-summary-grid"><div class="entry settings-summary-card" id="settings-account-security"><b>目前使用者</b><div class="muted">${escapeHtml(session.name)}｜${escapeHtml(session.status || session.email || "")}</div></div><div class="entry settings-summary-card"><b>工作身分</b><div class="muted">${escapeHtml(profileStatus)}</div><div class="source-path">目前工作任務：${escapeHtml(wp.defaultTask || "尚未設定")}｜有效月份：${escapeHtml(wp.taskEffectiveMonth || "尚未設定")}</div></div></div><section class="settings-account-security">${identitySetup}</section><section class="settings-preferences" id="settings-preferences"><div class="entry settings-preference-card"><b>Smart Auto Save</b><div class="muted">設定一修改即更新本機狀態，約 2 秒後自動同步 Cloud。</div></div><label class="settings-role-field" for="roleSet"><span>角色</span><select id="roleSet" class="input">${roleOptions}</select></label></section><section class="work-model-section settings-ecp-section"><div class="settings-section-heading"><div><h3>ECP 設定</h3><div class="muted">目前工作任務會作為 ECP 匯出的任務欄位來源。</div></div></div><div class="settings-form-grid"><label class="settings-form-field">ECP 負責人<input class="input" id="ecpOwner" value="${escapeHtml(profile?.ecpOwner || "")}" placeholder="例如：陳彥達-UU"></label><label class="settings-form-field">ECP 負責部門<input class="input" id="ecpDepartment" value="${escapeHtml(profile?.ecpDepartment || "")}" placeholder="例如：UU管理部"></label><div class="settings-form-field settings-field-wide"><span>目前工作任務（Current Active Task）</span>${ecpTaskList(tasks)}</div><div class="work-model-add settings-ecp-add"><input class="input" id="newEcpTask" placeholder="新增 ECP 任務，例如：採購案件處理"><button class="btn2" id="addEcpTask" type="button">＋ 新增 ECP 任務</button></div></div><div class="muted settings-help">快速紀錄仍可選「不指定 ECP 任務」。</div></section>${workSection}${advancedSection}</section>`;
}
...
function bindTasks() {
  ...
  document.querySelectorAll("[data-task-journal]").forEach(button => button.onclick = event => {
    event.stopPropagation();
    openTaskJournal(button.dataset.taskJournal);
  });
  document.querySelectorAll("[data-worktodo-open-task]").forEach(card => card.addEventListener("click", event => {
  ...
  document.querySelectorAll("[data-task-edit]").forEach(button => button.onclick = event => {
    event.stopPropagation();
    openTaskJournal(button.dataset.taskEdit);
  });
