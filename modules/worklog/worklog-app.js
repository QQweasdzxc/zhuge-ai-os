...
function saveLocalSnapshot() {
  // Cloud SSOT: business data is never persisted in browser storage.
  // Only authentication/session and non-business UI preferences may remain local.
...
function loadTaskDraft(taskId = "new") {
  return taskDraft && taskDraft.taskId === taskId ? taskDraft : null;
}

function saveTaskDraft(taskId = "new", draft = {}) {
  const next = { ...draft, taskId, updatedAt: new Date().toISOString() };
  taskDraft = next;
  return next;
}
