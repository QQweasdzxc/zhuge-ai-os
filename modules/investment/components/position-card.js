(function (root, factory) {
  ...
  function render(position, dependencies = {}) {
    ...
    return `<article class="investment-position-card ${trend}">
      <header><div><strong>${escape(position.symbol)}</strong><span>${escape(position.name)}</span></div><div class="investment-price"><small>今日</small><b>${format.number(position.lastPrice)}</b><em>${position.market === "US" ? "🇺🇸" : "🇹🇼"}</em></div></header>
