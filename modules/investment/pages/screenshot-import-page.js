(function (root, factory) {
  ...
  function renderConfirmedInput(session, escape) {
    ...
    return '<article class="investment-import-panel investment-import-confirmed-input" data-investment-confirmed-input><header class="investment-import-panel-heading"><div><p class="investment-eyebrow">PM Confirmed Input</p><h2>載入已確認的結構化持倉</h2><p>只接受已完成 GPT Assisted Recognition 且已由 PM 逐筆確認的既有結果；本操作不重新辨識、不建立圖片，也不會直接寫入 Cloud。</p></div><span class="investment-import-badge">瀏覽器記憶體</span></header><form data-investment-confirmed-input-form><label class="investment-import-confirmed-input-label"><span>PM Confirmed Snapshot JSON</span><textarea name="positionsJson" rows="12" required placeholder="{\"broker\":\"…\",\"snapshotAt\":\"YYYY-MM-DDTHH:MM\",\"source\":\"…\",\"idempotencyKey\":\"…\",\"positions\":[…]}"></textarea></label><div class="investment-import-write-actions"><button type="submit" class="investment-primary-link">載入並產生 Reconciliation</button><span class="investment-import-write-hint">載入後仍須逐筆確認 Preview，最後只能透過受控安全驗證寫入 Snapshot。</span></div></form></article>';
  }

  function renderSensitiveWriteStepUp(stepUp, escape) {
    const state = stepUp || {};
    const busy = ["preparing", "enrolling", "verifying"].includes(state.status);
    const error = state.error
      ? '<div class="investment-import-step-up-error" role="alert">' + escapeText(state.error, escape) + '</div>'
      : "";
    let content = '<div class="investment-import-step-up-progress" role="status">正在準備安全驗證…</div>';
    if (state.mode === "enrollment_required") {
      content = '<p>第一次使用前，請先設定 Google Authenticator。</p><button type="button" class="investment-primary-link" data-investment-sensitive-write-enroll ' + (busy ? 'disabled' : '') + '>開始設定驗證器</button>';
    } else if (state.mode === "enroll") {
      content = '<div class="investment-import-step-up-enroll"><div><img class="investment-import-step-up-qr" src="' + escapeText(state.qrCode || "", escape) + '" alt="Google Authenticator 設定 QR Code"><p>用 Google Authenticator 掃描 QR Code，再輸入 App 顯示的 6 位數驗證碼。</p><details><summary>無法掃描？查看設定金鑰</summary><code>' + escapeText(state.secret || "", escape) + '</code></details></div>' + renderSensitiveWriteCodeForm(state, escape, busy) + '</div>';
    } else if (state.mode === "challenge") {
      content = '<p>這次操作需要安全驗證。請開啟 Google Authenticator，輸入目前顯示的 6 位數驗證碼。</p>' + renderSensitiveWriteCodeForm(state, escape, busy);
    } else if (state.mode === "error") {
      content = '<button type="button" class="investment-secondary-link" data-investment-sensitive-write-retry>重新準備安全驗證</button>';
    }
    return '<div class="investment-import-step-up" data-investment-sensitive-write-step-up-panel><div class="investment-import-step-up-heading"><strong>需要安全驗證</strong><span>驗證成功後會繼續原本的 Snapshot 寫入，不需要重新上傳或確認資料。</span></div>' + error + content + '</div>';
  }

  function renderSensitiveWriteCodeForm(state, escape, busy) {
    return '<form class="investment-import-step-up-form" data-investment-sensitive-write-step-up><label><span>Google Authenticator 6 位數驗證碼</span><input name="code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" placeholder="輸入 6 位數驗證碼" required ' + (busy ? 'disabled' : '') + '></label><input type="hidden" name="factorId" value="' + escapeText(state.factorId || "", escape) + '"><button type="submit" class="investment-primary-link" ' + (busy ? 'disabled' : '') + '>' + (busy ? "驗證中…" : "驗證並繼續") + '</button></form>';
  }
  ...
  function renderSnapshotWrite(session, comparison, dependencies, escape) {
    ...
    const stepUpActive = status === "step-up-required";
    ...
    const disabled = busy || stepUpActive || blocked;
    const formDisabled = busy || stepUpActive;
    const badge = blocked ? "等待完整資料" : busy ? "寫入中…" : stepUpActive ? "需要安全驗證" : "受控寫入";
    const stepUp = stepUpActive ? renderSensitiveWriteStepUp(write.stepUp, escape) : "";
    return '<article class="investment-import-panel investment-import-write-panel" data-investment-snapshot-write><header class="investment-import-panel-heading"><div><p class="investment-eyebrow">04 · Controlled Snapshot Write</p><h2>寫入 Broker Position Snapshot</h2><p>只有 Preview Confirmed、完整範圍且每筆欄位可驗證時，才會先完成安全驗證，再以目前帳戶的投資組合寫入；不會改寫 Transaction 或 Legacy Opening Position。</p></div><span class="investment-import-badge ' + (blocked ? 'is-pending' : '') + '">' + badge + '</span></header><div class="investment-import-write-contract"><span>Snapshot：append-only evidence</span><span>Transaction mutation：0</span><span>Legacy data：保留</span></div>' + stepUp + '<form class="investment-import-write-form" data-investment-snapshot-write-form><div class="investment-import-write-grid"><label><span>Broker</span><input name="broker" value="' + escapeText(broker, escape) + '" maxlength="120" required ' + (formDisabled ? 'disabled' : '') + '></label><label><span>Snapshot 時間（Asia/Taipei）</span><input type="datetime-local" name="snapshotAt" value="' + escapeText(form.snapshotAt || "", escape) + '" required ' + (formDisabled ? 'disabled' : '') + '></label><label><span>Source</span><input name="source" value="' + escapeText(source, escape) + '" maxlength="160" required ' + (formDisabled ? 'disabled' : '') + '></label><label><span>Idempotency Key</span><input name="idempotencyKey" value="' + escapeText(idempotencyKey, escape) + '" minlength="8" maxlength="240" required ' + (formDisabled ? 'disabled' : '') + '></label></div><div class="investment-import-write-actions"><button type="submit" class="investment-import-write-button" ' + (disabled ? 'disabled' : '') + '>確認並寫入 ' + eligibility.rows.length + ' 筆 Snapshot</button><span class="investment-import-write-hint">' + escapeText(blocked ? eligibility.reason : "時間會以 Asia/Taipei 轉換後送交受控流程。", escape) + '</span></div></form>' + statusMessage + '</article>';
  }
  ...
  function renderReconciliation(state, dependencies) {
    ...
    const confirmed = session.preview?.status === "confirmed" ? '<div class="investment-import-confirmed" role="status"><strong>Preview Confirmed／Ready for Controlled Write</strong><span>預覽已確認；正式資料只會由下方受控安全流程寫入，不會直接修改 Transaction 或 Legacy Opening Position。</span></div>' : "";
  ...
  function render(state, dependencies = {}) {
    ...
    return '<section class="investment-screenshot-import" data-investment-import-page><div class="investment-page-heading"><div><p class="investment-eyebrow">Screenshot Investment Import · Recognition + Snapshot</p><h1>持股截圖匯入</h1><p>把最新持股畫面帶進可追溯的辨識、Reconciliation 與受控安全 Snapshot 流程；不會用截圖差異偽造交易。</p></div><span class="investment-pill">PNG · JPG · WebP</span></div>' +
      '<div class="investment-import-contract" data-investment-import-contract><div><strong>安全邊界</strong><p>原圖只在目前瀏覽器記憶體與受控辨識請求中存在；不建立 Storage、不把原圖寫入 Cloud、不在瀏覽器保存 API Key。只有確認後的結構化 Position Evidence 才能經受控安全流程寫入 Snapshot。</p></div><span>Cloud：受控 Snapshot</span></div>' +
