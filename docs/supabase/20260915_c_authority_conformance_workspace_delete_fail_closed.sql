-- Module C checker classification follow-up for populated Workspace Delete.
-- board_instance_delete_workspace is the canonical empty-workspace writer;
-- its populated branch is fail-closed and is not an alternate task writer.
...
declare
  ...
  v_adoption_candidate_count integer := 0;
  v_adoption_resolution text := 'unresolved';
  ...
  v_canonical_provisioning boolean := false;
  ...
  v_unscoped_attachment_orphan_count integer := 0;
  v_unscoped_checklist_orphan_count integer := 0;
  ...
  -- These are shared Board / governance contracts, not alternate C
  -- authorities.  Their presence is retained in the evidence inventory,
  -- but body-level writes alone must not turn a legitimate shared capability
  -- into a C Health finding.
  v_shared_capability_functions text[] := array[
    'allocate_board_task_work_code',
    'board_add_task_checklist_item',
    'board_add_task_progress_note',
    'board_cancel_delete_progress_attachment',
    'board_cancel_delete_task_attachment',
    'board_claim_next_task',
    'board_claim_specific_task',
    'board_complete_task_attachment',
    'board_create_checklist_item',
    'board_create_instance',
    'board_create_task',
    'board_create_workspace',
    'board_delete_task_checklist_item',
    'board_delete_task_progress_note',
    'board_edit_task_progress_note',
    'board_finalize_delete_progress_attachment',
    'board_finalize_delete_task_attachment',
    'board_get_workspace_notification_settings',
    'board_prepare_progress_note_attachment',
    'board_prepare_task_attachment',
    'board_reclaim_expired_task',
    'board_release_task_claim',
    'board_renew_task_claim',
    'board_request_delete_progress_attachment',
    'board_request_delete_task_attachment',
    ...
    'board_pm_acceptance_from_qjc_drop',
    'board_pm_qa_fail_requeue',
    'board_provision_consumer'
  ];
  v_domain_extension_functions text[] := array[
    'repair_investment_ivtk_identity',
    'sync_investment_ivtk_projection'
  ];
  v_governance_functions text[] := array[
    'board_governance_action',
    'board_orchestrate_developer_qa',
    'board_orchestrate_engineering_review',
    'board_pm_acceptance_from_qjc_drop',
    'board_pm_qa_fail_requeue',
    'board_reconcile_qjc_task_to_co_ready',
    'execute_engineering_governance_write'
  ];
  v_retired_route_functions text[] := array[
    'board_instance_move_task_workspace',
    'board_instance_transition_task',
    'board_c_reconcile_workspace_decision',
    'board_c_workflow_reconcile_legacy_card_v2',
    'board_reconcile_completion_lifecycle',
    'board_reconcile_pm_acceptance_lifecycle',
  ...
  v_is_legacy boolean;
  v_is_worklog_compatibility boolean;
  v_is_domain_extension boolean;
  v_is_shared_capability boolean;
  v_is_governance boolean;
  v_is_retired_route boolean;
  v_c_authority_relevant boolean;
  v_c_health_relevant boolean;
  v_trigger_c_health_relevant boolean;
  v_surface_classification text;
  v_evidence_key text;
  v_definition text;
  v_capability text;
...
begin
  ...
  if v_release_found then
    -- Prefer the instance UUID.  A scope alias is only acceptable when it
    -- resolves to exactly one adopted record; never pick an arbitrary alias
    -- when several consumers share the same application scope.
    select count(*)
      into v_adoption_candidate_count
      from jsonb_each(coalesce(v_release.consumer_adoptions, '{}'::jsonb)) adoption
     where adoption.key = p_board_instance_id::text
       and adoption.value->>'status' = 'adopted';
    if v_adoption_candidate_count = 1 then
      select adoption.key, adoption.value
        into v_release_adoption_key, v_release_adoption
        from jsonb_each(coalesce(v_release.consumer_adoptions, '{}'::jsonb)) adoption
       where adoption.key = p_board_instance_id::text
         and adoption.value->>'status' = 'adopted';
      v_adoption_resolution := 'instance_uuid';
    elsif v_adoption_candidate_count = 0
      and v_expected_adoption_key is not null then
      select count(*)
        into v_adoption_candidate_count
        from jsonb_each(coalesce(v_release.consumer_adoptions, '{}'::jsonb)) adoption
      ...
      if v_adoption_candidate_count = 1 then
        select adoption.key, adoption.value
         ...
         where adoption.value->>'status' = 'adopted'
           and replace(lower(adoption.key), '_', '-') =
               replace(lower(v_expected_adoption_key), '_', '-');
        v_adoption_resolution := 'canonical_scope_alias';
      elsif v_adoption_candidate_count > 1 then
        v_adoption_resolution := 'ambiguous_scope_alias';
      end if;
    end if;
    v_release_adopted := v_adoption_resolution in ('instance_uuid', 'canonical_scope_alias')
  ...
  -- Provisioning is a first-class C contract, but it is not a card,
  -- workspace, movement, or lifecycle writer.  Verify the exact v2 contract
  -- separately so its implementation body is not misclassified as an
  -- alternate card writer.
  select exists (
           select 1
             from pg_proc p
             join pg_namespace n on n.oid = p.pronamespace
            where n.nspname = 'public'
              and p.oid = to_regprocedure(
                'public.board_provision_c_consumer_v2(text,text,text,text,jsonb,jsonb,text)'
              )::oid
              and has_function_privilege('authenticated', p.oid, 'execute')
         )
    into v_canonical_provisioning;
  ...
  loop
    ...
    v_is_worklog_compatibility := v_function.function_name ~* '^worktodo_'
      or v_writes_legacy;
    v_is_domain_extension := v_function.function_name = any(v_domain_extension_functions);
    v_is_shared_capability := v_function.function_name = any(v_shared_capability_functions);
    v_is_governance := v_function.function_name = any(v_governance_functions);
    v_is_retired_route := v_function.function_name = any(v_retired_route_functions);
    v_is_legacy := v_is_worklog_compatibility
      or v_is_retired_route
      or v_function.function_name ~* '(legacy|old|deprecated|compatibility|migration|fallback|v1)';
    -- C Health is about the C authority boundary, not every function that
    -- happens to write a C-shaped table.  Shared Board capabilities,
    -- governance routes, Investment projections, and WorkLog compatibility
    -- are retained as evidence but are not alternate C authorities.
    v_c_authority_relevant := v_writes_c_owned
      and not v_is_shared_capability
      and not v_is_domain_extension
      and not v_is_governance
      and not v_is_worklog_compatibility;
    v_c_health_relevant := v_is_canonical or v_c_authority_relevant;
    v_is_alternate := v_application_reachable
      and v_c_authority_relevant
      and not v_is_canonical;

    if v_function.function_name in ('board_provision_consumer', 'board_provision_c_consumer_v2') then
      v_capability := 'provisioning';
    elsif v_is_domain_extension then
      v_capability := 'domain_extension';
    elsif v_is_governance then
      v_capability := 'governance';
    elsif v_function.function_name ~* '(move|movement)' then
    ...
    v_target_relation := case
      when v_is_domain_extension then 'investment-domain'
      when v_is_worklog_compatibility then 'worklog-compatibility'
      when v_writes_c_owned then 'c-owned'
      when v_writes_legacy then 'legacy-worktodo'
      else 'read-or-unresolved'
    end;

    v_evidence_key := format(
      '%s.%s(%s)',
      v_function.schema_name,
      v_function.function_name,
      v_function.identity_arguments
    );
    v_surface_classification := case
      when v_is_alternate then 'ALTERNATE'
      when v_is_canonical then 'CANONICAL'
      when v_function.function_name = 'board_provision_consumer' then 'COMPATIBILITY_PROVISIONING'
      when v_is_domain_extension then 'DOMAIN_EXTENSION'
      when v_is_governance then 'GOVERNANCE'
      when v_is_worklog_compatibility and v_application_reachable then 'LEGACY_COMPATIBILITY_REACHABLE'
      when v_is_worklog_compatibility then 'HISTORICAL_COMPATIBILITY'
      when v_is_retired_route then 'HISTORICAL_COMPATIBILITY'
      when v_is_shared_capability then 'SHARED_CAPABILITY'
      else 'UNRESOLVED'
    end;

    if v_writes_c_owned or v_writes_legacy or v_is_legacy
       or v_is_shared_capability or v_is_domain_extension then
      v_function_surface := jsonb_build_object(
        ...
        'authority_scope', case
          when v_function.function_name = 'board_provision_consumer' then 'compatibility-provisioning'
          when v_is_domain_extension then 'investment-domain'
          when v_is_worklog_compatibility then 'worklog-compatibility'
          when v_is_governance then 'governance'
          when v_is_shared_capability then 'shared-capability'
          when v_c_authority_relevant then 'c-authority'
          else 'unresolved'
        end,
        'c_health_relevant', v_c_health_relevant,
        'health_relevant', v_c_health_relevant,
        ...
        'classification', v_surface_classification
      );
    ...
    if v_application_reachable
      ...
      -- WorkLog / user_tasks compatibility is intentionally outside the C
      -- Health scope.  It remains visible in legacy_routes and the surface
      -- inventory, but it cannot make a C Consumer unhealthy by itself.
    elsif v_application_reachable
       and v_c_authority_relevant
       and v_is_legacy
       and not v_is_canonical then
      -- A legacy-named route that still writes current C state is a real C
      -- authority conflict if its application Execute surface is restored.
  ...
  loop
    ...
    v_trigger_c_health_relevant := v_trigger.table_name <> 'user_tasks';
    ...
    v_trigger_alternate := v_trigger_enabled
      and v_trigger_c_health_relevant
    ...
    v_trigger_surface := jsonb_build_object(
      ...
      'evidence_key', format(
        ...
        v_trigger.identity_arguments
      ),
      'canonical', v_trigger_canonical,
      'legacy', v_trigger_legacy,
      'authority_scope', case
        when v_trigger_canonical then 'c-authority'
        when not v_trigger_c_health_relevant then 'worklog-compatibility'
        when v_trigger_legacy then 'c-legacy'
        else 'c-authority'
      end,
      'c_health_relevant', v_trigger_c_health_relevant,
      'health_relevant', v_trigger_c_health_relevant,
      'classification', case
    ...
    if v_trigger_enabled and v_trigger_alternate then
      ...
      if v_trigger_c_health_relevant then
        v_legacy_current_route := true;
      end if;
  ...
  -- fail because a global orphan exists elsewhere.
  select count(*)
    into v_unscoped_attachment_orphan_count
  ...
  select count(*)
    into v_unscoped_checklist_orphan_count
  ...
  v_data_isolation_status := case
    ...
    when v_historical_activity_unknown_count > 0
      or v_unscoped_attachment_orphan_count > 0
      or v_unscoped_checklist_orphan_count > 0 then 'unknown'
    else 'pass'
  end;
  ...
  elsif v_historical_activity_unknown_count > 0
     or v_unscoped_attachment_orphan_count > 0
     or v_unscoped_checklist_orphan_count > 0 then
    v_findings := v_findings || jsonb_build_array(jsonb_build_object(
      'rule', 'historical-parentless-activity',
      'status', 'unknown',
      'reason', 'Some historical or orphan child evidence cannot be assigned to this Consumer without a parent Board task.',
      'evidence', jsonb_build_object(
        'unmatched_activity_count', v_historical_activity_unknown_count,
        'unscoped_attachment_orphan_count', v_unscoped_attachment_orphan_count,
        'unscoped_checklist_orphan_count', v_unscoped_checklist_orphan_count
      ),
      'affected_capability', 'data_isolation',
  ...
  -- WorkLog/user_tasks compatibility writers are intentionally outside the
  -- Module C Health scope.  Keep their count in legacy_routes and the raw
  -- surface inventory, but do not turn compatible historical reachability
  -- into a C Authority finding.
  ...
  v_runtime_route_reason := case
    ...
    else 'Cloud static catalog found no application-reachable alternate C writer, C legacy route, or alternate trigger.'
  end;
  ...
  return jsonb_build_object(
    ...
    'authority', jsonb_build_object(
      'provisioning', case when v_canonical_provisioning then 'module-c-canonical-contract' else 'unverified' end,
    ...
    'adoption', jsonb_build_object(
      'module_release_adopted', v_release_adopted,
      'module_adoption_key', v_release_adoption_key,
      'adoption_resolution', v_adoption_resolution,
      'adoption_candidate_count', v_adoption_candidate_count,
    ...
    'data_isolation', jsonb_build_object(
      ...
      'attachment_orphans', v_attachment_orphan_count,
      'checklist_orphans', v_checklist_orphan_count,
      'unscoped_attachment_orphans', v_unscoped_attachment_orphan_count,
      'unscoped_checklist_orphans', v_unscoped_checklist_orphan_count,
    ...
    'legacy_routes', jsonb_build_object(
      ...
      'worklog_compatibility_outside_c_health', true,
    ...
    'capabilities', jsonb_build_object(
      'provisioning', jsonb_build_object(
        'status', case when v_canonical_provisioning then 'pass' else 'fail' end,
        'canonical_contract', v_canonical_provisioning,
        'contract', 'public.board_provision_c_consumer_v2(text,text,text,text,jsonb,jsonb,text)'
      ),
    ...
    'checks', jsonb_build_object(
      ...
      'provisioning_contract', case when v_canonical_provisioning then 'pass' else 'fail' end,
