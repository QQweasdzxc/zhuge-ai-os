-- Investment IVTK projection market routing repair
-- Scope: route existing/new position projections to the formal TW/US workspaces.
-- This replaces only the projection function. It does not alter Investment
-- financial data, position identity, card identity, or workspace definitions.
...
begin
  ...
  -- The US workspace predates a stable workspace_key, so its formal product
  -- identity is resolved by its owner-scoped name rather than a guessed key.
  ...
  if v_stocks_workspace.id is null
     ...
     or v_watchlist_workspace.id is null then
  ...
  loop
    ...
    if upper(trim(coalesce(v_position.market, ''))) = 'US' then
      ...
      raise exception using
        errcode = 'P0002',
        message = 'INVESTMENT_IVTK_MARKET_WORKSPACE_REQUIRED';
    end if;
    ...
    if not v_link_found then
      ...
      if not v_relinked and (not v_link.active or v_link.portfolio_id is distinct from v_position.portfolio_id) then
        update public.investment_ivtk_card_links link
      ...
      -- Repair only the projection's board placement. Position, card identity,
      -- and all Investment financial data remain untouched.
