...
<!-- SECTION:PLAN:BEGIN -->
1. Reuse the existing formal WorkTodo Board Instance and board_tasks data; do not provision a second board or clone any cards. 2. Route the official WorkTodo entry through the shared Module C Instance Service and shared task/action adapters. 3. Add a single shared-runtime read-only comparison mode for WorkTodo（舊） so the legacy entry cannot write. 4. Preserve the existing Board/Card/WLTK/child-data identity and C lifecycle context; fail closed when the existing instance lacks a published workflow. 5. Add focused route/writer/read-only regression coverage, run the existing full suite, and record the PM Runtime QA handoff without retiring legacy data or entries.
<!-- SECTION:PLAN:END -->
...
2026-09-12 PM execution: same-data runtime cutover authorized from Board Instance 0b2b5c4e-6767-4792-a97a-d2ddf42e60da; old data and protected Candidate remain rollback boundaries.
<!-- SECTION:NOTES:END -->
