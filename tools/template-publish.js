...
const PARITY_ENGINE_FILE = path.join(ROOT, "shared", "components", "template-parity-engine.js");
const HTML_FILES = [
...
const CANONICAL_TEMPLATE_FILES = [
  ...
  "shared/components/template-parity-engine.js",
...
function publishedSemanticSnapshot(identity) {
  ...
  return {
    ...
    inventory: parity.canonicalInventory(),
    behaviorContract: parity.canonicalBehaviorContract()
  };
}

function renderModuleSource(record) {
...
function renderModule(record) {
  const source = renderModuleSource(record);
  return source
    .replace(
      '    return { version: String(identity.version || ""), build: String(identity.build || "") };',
      [
...
function createRecord() {
  ...
  return {
    ...
    build: identity.build,
    developmentSourceCommit: commit,
    developmentSourceFingerprint: fingerprint,
    sourceCommit: commit,
    sourceDirty: dirty,
    sourceFingerprint: fingerprint,
    publishedSnapshot,
...
function check() {
  ...
  expect(record.developmentSourceCommit && /^[0-9a-f]{40}$/i.test(record.developmentSourceCommit), "developmentSourceCommit must be generated");
  let developmentCommitReachable = false;
  try {
    git(["cat-file", "-e", `${record.developmentSourceCommit}^{commit}`]);
    git(["merge-base", "--is-ancestor", record.developmentSourceCommit, "HEAD"]);
    developmentCommitReachable = true;
  } catch (_error) {
    developmentCommitReachable = false;
  }
  expect(developmentCommitReachable, "developmentSourceCommit must resolve from current source history");
  expect(record.developmentSourceFingerprint && /^[0-9a-f]{64}$/i.test(record.developmentSourceFingerprint), "developmentSourceFingerprint must be generated");
  expect(record.developmentSourceFingerprint === sourceFingerprint(), "developmentSourceFingerprint must match canonical source");
